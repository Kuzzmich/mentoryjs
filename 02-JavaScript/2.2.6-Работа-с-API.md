# 2.2.6 Работа с API (fetch)

Fetch API — современный способ делать HTTP-запросы в JavaScript. Это основа взаимодействия фронтенда с бэкендом.

---

## 🎯 Что ты узнаешь

- Что такое HTTP и REST API
- Как работает Fetch API
- HTTP методы: GET, POST, PUT, DELETE
- Работа с заголовками и телом запроса
- Обработка ответов (JSON, текст, blob)
- Правильная обработка ошибок
- CORS и способы работы с ним
- Практическая интеграция с реальными API

---

## 📖 Что такое API

**API (Application Programming Interface)** — интерфейс для взаимодействия программ друг с другом.

### REST API

**REST** — архитектурный стиль для создания веб-сервисов.

```
GET    /api/users          # Получить список пользователей
GET    /api/users/123      # Получить пользователя с id=123
POST   /api/users          # Создать нового пользователя
PUT    /api/users/123      # Обновить пользователя с id=123
DELETE /api/users/123      # Удалить пользователя с id=123
```

**Основные принципы REST:**
- Каждый ресурс имеет уникальный URL
- Используются стандартные HTTP методы
- Stateless — каждый запрос независим
- Данные обычно передаются в JSON формате

---

## 📖 Fetch API — Основы

### Простой GET запрос

```javascript
// Простейший запрос
fetch('https://api.example.com/data')
  .then(response => response.json())
  .then(data => console.log(data))
  .catch(error => console.error('Ошибка:', error));
```

### С async/await (рекомендуется)

```javascript
async function fetchData() {
  try {
    const response = await fetch('https://api.example.com/data');
    const data = await response.json();
    console.log(data);
  } catch (error) {
    console.error('Ошибка:', error);
  }
}
```

### Структура Response

```javascript
const response = await fetch('https://api.example.com/data');

console.log(response.status);      // 200
console.log(response.statusText);  // "OK"
console.log(response.ok);          // true (для статусов 200-299)
console.log(response.headers);     // Headers объект
console.log(response.url);         // URL запроса
```

---

## 📖 HTTP методы

### GET — Получение данных

```javascript
// Без параметров
const response = await fetch('https://api.example.com/users');

// С параметрами в URL
const userId = 123;
const response = await fetch(`https://api.example.com/users/${userId}`);

// С query параметрами
const params = new URLSearchParams({
  search: 'John',
  limit: 10,
  page: 1
});
const response = await fetch(`https://api.example.com/users?${params}`);
// URL: https://api.example.com/users?search=John&limit=10&page=1
```

### POST — Создание данных

```javascript
const newUser = {
  name: 'John Doe',
  email: 'john@example.com'
};

const response = await fetch('https://api.example.com/users', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
  body: JSON.stringify(newUser)
});

const data = await response.json();
console.log('Создан пользователь:', data);
```

### PUT — Обновление данных

```javascript
const updatedUser = {
  name: 'John Smith',
  email: 'john.smith@example.com'
};

const response = await fetch('https://api.example.com/users/123', {
  method: 'PUT',
  headers: {
    'Content-Type': 'application/json'
  },
  body: JSON.stringify(updatedUser)
});
```

### DELETE — Удаление данных

```javascript
const response = await fetch('https://api.example.com/users/123', {
  method: 'DELETE'
});

if (response.ok) {
  console.log('Пользователь удалён');
}
```

### PATCH — Частичное обновление

```javascript
// Обновляем только email
const response = await fetch('https://api.example.com/users/123', {
  method: 'PATCH',
  headers: {
    'Content-Type': 'application/json'
  },
  body: JSON.stringify({ email: 'newemail@example.com' })
});
```

---

## 📖 Работа с заголовками

### Отправка заголовков

```javascript
const response = await fetch('https://api.example.com/data', {
  headers: {
    'Content-Type': 'application/json',
    'Authorization': 'Bearer YOUR_TOKEN',
    'Accept': 'application/json',
    'X-Custom-Header': 'custom-value'
  }
});
```

### Чтение заголовков ответа

```javascript
const response = await fetch('https://api.example.com/data');

// Получить конкретный заголовок
console.log(response.headers.get('Content-Type'));

// Перебрать все заголовки
for (let [key, value] of response.headers) {
  console.log(`${key}: ${value}`);
}

// Проверить наличие заголовка
if (response.headers.has('X-Rate-Limit')) {
  console.log('Rate limit:', response.headers.get('X-Rate-Limit'));
}
```

---

## 📖 Обработка разных типов данных

### JSON (самый частый)

```javascript
const response = await fetch('https://api.example.com/data');
const data = await response.json();
console.log(data);
```

### Текст

```javascript
const response = await fetch('https://example.com/text');
const text = await response.text();
console.log(text);
```

### Blob (файлы, изображения)

```javascript
const response = await fetch('https://example.com/image.jpg');
const blob = await response.blob();

// Создать URL для отображения
const imageUrl = URL.createObjectURL(blob);
document.getElementById('img').src = imageUrl;
```

### FormData (загрузка файлов)

```javascript
const formData = new FormData();
formData.append('file', fileInput.files[0]);
formData.append('description', 'My file');

const response = await fetch('https://api.example.com/upload', {
  method: 'POST',
  body: formData
  // Не указывай Content-Type! Браузер установит его автоматически
});
```

---

## 📖 Обработка ошибок

### Два типа ошибок

1. **Network errors** — сеть недоступна, сервер не отвечает
2. **HTTP errors** — сервер ответил с ошибкой (404, 500, etc.)

```javascript
async function fetchWithErrorHandling(url) {
  try {
    const response = await fetch(url);

    // Fetch НЕ бросает ошибку при HTTP 4xx/5xx
    // Нужно проверять response.ok вручную
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }

    const data = await response.json();
    return data;
  } catch (error) {
    // Обработка network errors и наших HTTP errors
    console.error('Ошибка запроса:', error);
    throw error;
  }
}
```

### Детальная обработка статусов

```javascript
async function fetchData(url) {
  try {
    const response = await fetch(url);

    if (response.status === 200) {
      return await response.json();
    } else if (response.status === 404) {
      throw new Error('Ресурс не найден');
    } else if (response.status === 401) {
      throw new Error('Требуется авторизация');
    } else if (response.status === 403) {
      throw new Error('Доступ запрещён');
    } else if (response.status === 500) {
      throw new Error('Ошибка сервера');
    } else {
      throw new Error(`Неизвестная ошибка: ${response.status}`);
    }
  } catch (error) {
    if (error.name === 'TypeError') {
      // Network error
      throw new Error('Проблемы с сетью. Проверьте подключение.');
    }
    throw error;
  }
}
```

### Обработка JSON ошибок от сервера

```javascript
async function fetchData(url) {
  const response = await fetch(url);

  if (!response.ok) {
    // Сервер может вернуть JSON с описанием ошибки
    const errorData = await response.json();
    throw new Error(errorData.message || `HTTP ${response.status}`);
  }

  return await response.json();
}

// Использование
try {
  const data = await fetchData('https://api.example.com/data');
} catch (error) {
  console.error('Ошибка:', error.message);
}
```

---

## 📖 Timeout и AbortController

### Timeout для запросов

```javascript
async function fetchWithTimeout(url, timeout = 5000) {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), timeout);

  try {
    const response = await fetch(url, {
      signal: controller.signal
    });
    clearTimeout(timeoutId);
    return await response.json();
  } catch (error) {
    if (error.name === 'AbortError') {
      throw new Error('Запрос превысил время ожидания');
    }
    throw error;
  }
}

// Использование
try {
  const data = await fetchWithTimeout('https://api.example.com/data', 3000);
} catch (error) {
  console.error(error.message);
}
```

### Отмена запроса

```javascript
const controller = new AbortController();

// Начинаем запрос
fetch('https://api.example.com/data', {
  signal: controller.signal
})
  .then(response => response.json())
  .then(data => console.log(data))
  .catch(error => {
    if (error.name === 'AbortError') {
      console.log('Запрос отменён');
    }
  });

// Отменяем запрос (например, при уходе со страницы)
controller.abort();
```

---

## 📖 CORS (Cross-Origin Resource Sharing)

### Что такое CORS

**CORS** — механизм безопасности браузера, ограничивающий запросы к другим доменам.

```javascript
// Ваш сайт: https://mysite.com
// API: https://api.example.com

// Может не работать из-за CORS!
fetch('https://api.example.com/data')
  .then(response => response.json())
  .catch(error => console.error('CORS error:', error));
```

**Ошибка в консоли:**
```
Access to fetch at 'https://api.example.com/data' from origin
'https://mysite.com' has been blocked by CORS policy
```

### Решения CORS

1. **Настройка на сервере** (лучший вариант):
```javascript
// Сервер должен отправить заголовки:
Access-Control-Allow-Origin: https://mysite.com
// или
Access-Control-Allow-Origin: *
```

2. **CORS Proxy** (для разработки):
```javascript
const proxyUrl = 'https://cors-anywhere.herokuapp.com/';
const targetUrl = 'https://api.example.com/data';

fetch(proxyUrl + targetUrl)
  .then(response => response.json());
```

3. **JSONP** (устаревший метод):
```javascript
// Работает только для GET запросов
// Не рекомендуется для новых проектов
```

4. **Backend Proxy**:
```javascript
// Делать запросы через свой сервер
// Фронтенд -> Ваш сервер -> Внешнее API
fetch('https://mysite.com/api/proxy?url=https://api.example.com/data')
```

---

## 🌤️ Практика: Погодное приложение

### Задание 2.2.6.1: Обёртка для Fetch API

Создай универсальную функцию для работы с API.

**Задача:** Создай функцию `apiRequest`, которая:
- Обрабатывает все HTTP методы
- Автоматически добавляет заголовки
- Правильно обрабатывает ошибки
- Поддерживает timeout

**Стартовый код:**

```javascript
// api-wrapper.js

/**
 * Универсальная функция для API запросов
 * TODO: Реализуй функцию с поддержкой:
 * - HTTP методов (GET, POST, PUT, DELETE)
 * - Автоматического добавления заголовков
 * - Timeout через AbortController
 * - Обработки ошибок (сеть + HTTP статусы)
 */
async function apiRequest(url, options = {}) {
  // Твоя реализация
  // Структура options: { method, body, headers, timeout }
}

// TODO: Создай удобные хелперы
// export const api = {
//   get: (url, options) => apiRequest(url, { ...options, method: 'GET' }),
//   post: (url, body, options) => apiRequest(url, { ...options, method: 'POST', body }),
//   put: (url, body, options) => apiRequest(url, { ...options, method: 'PUT', body }),
//   delete: (url, options) => apiRequest(url, { ...options, method: 'DELETE' })
// };

// Примеры использования с реальным тестовым API (JSONPlaceholder):

// GET запрос - получить список пользователей
// const users = await apiRequest('https://jsonplaceholder.typicode.com/users');
// console.log(users);

// GET запрос - получить конкретного пользователя
// const user = await apiRequest('https://jsonplaceholder.typicode.com/users/1');
// console.log(user);

// POST запрос - создать нового пользователя
// const newUser = await apiRequest('https://jsonplaceholder.typicode.com/users', {
//   method: 'POST',
//   body: { name: 'John Doe', email: 'john@example.com' }
// });
// console.log(newUser);

// PUT запрос - обновить пользователя
// const updated = await apiRequest('https://jsonplaceholder.typicode.com/users/1', {
//   method: 'PUT',
//   body: { name: 'Jane Doe', email: 'jane@example.com' }
// });

// DELETE запрос - удалить пользователя
// await apiRequest('https://jsonplaceholder.typicode.com/users/1', {
//   method: 'DELETE'
// });

// С timeout - прервать если долго
// const data = await apiRequest('https://jsonplaceholder.typicode.com/posts', {
//   timeout: 3000
// });
```

**Тестовый HTML для проверки:**

```html
<!DOCTYPE html>
<html lang="ru">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Тест API Wrapper</title>
  <style>
    body {
      font-family: Arial, sans-serif;
      max-width: 800px;
      margin: 50px auto;
      padding: 20px;
    }
    button {
      margin: 10px 5px;
      padding: 10px 20px;
      font-size: 16px;
      cursor: pointer;
      border: none;
      border-radius: 4px;
      background: #667eea;
      color: white;
    }
    button:hover {
      background: #5568d3;
    }
    #output {
      margin-top: 20px;
      padding: 20px;
      background: #f5f5f5;
      border-radius: 8px;
      white-space: pre-wrap;
      font-family: monospace;
      max-height: 500px;
      overflow-y: auto;
    }
  </style>
</head>
<body>
  <h1>API Wrapper - Тест</h1>

  <div>
    <button onclick="testGet()">GET /users</button>
    <button onclick="testPost()">POST /users</button>
    <button onclick="testPut()">PUT /users/1</button>
    <button onclick="testDelete()">DELETE /users/1</button>
    <button onclick="testError()">Test 404 Error</button>
    <button onclick="testTimeout()">Test Timeout</button>
  </div>

  <div id="output">Результаты появятся здесь...</div>

  <script type="module">
    // TODO: Импортируй свою функцию apiRequest
    // import apiRequest from './api-wrapper.js';

    window.log = (message) => {
      const output = document.getElementById('output');
      output.textContent = typeof message === 'object'
        ? JSON.stringify(message, null, 2)
        : message;
    };

    window.testGet = async () => {
      try {
        log('Загрузка пользователей...');
        const users = await apiRequest('https://jsonplaceholder.typicode.com/users');
        log(users);
      } catch (error) {
        log('Ошибка: ' + error.message);
      }
    };

    window.testPost = async () => {
      try {
        log('Создание пользователя...');
        const newUser = await apiRequest('https://jsonplaceholder.typicode.com/users', {
          method: 'POST',
          body: {
            name: 'John Doe',
            email: 'john@example.com',
            username: 'johndoe'
          }
        });
        log(newUser);
      } catch (error) {
        log('Ошибка: ' + error.message);
      }
    };

    window.testPut = async () => {
      try {
        log('Обновление пользователя...');
        const updated = await apiRequest('https://jsonplaceholder.typicode.com/users/1', {
          method: 'PUT',
          body: {
            name: 'Jane Smith',
            email: 'jane@example.com'
          }
        });
        log(updated);
      } catch (error) {
        log('Ошибка: ' + error.message);
      }
    };

    window.testDelete = async () => {
      try {
        log('Удаление пользователя...');
        await apiRequest('https://jsonplaceholder.typicode.com/users/1', {
          method: 'DELETE'
        });
        log('Пользователь успешно удалён');
      } catch (error) {
        log('Ошибка: ' + error.message);
      }
    };

    window.testError = async () => {
      try {
        log('Запрос несуществующего ресурса...');
        await apiRequest('https://jsonplaceholder.typicode.com/users/999999');
      } catch (error) {
        log('Ошибка (ожидаемо): ' + error.message);
      }
    };

    window.testTimeout = async () => {
      try {
        log('Тест timeout (1ms)...');
        await apiRequest('https://jsonplaceholder.typicode.com/users', {
          timeout: 1
        });
      } catch (error) {
        log('Ошибка timeout (ожидаемо): ' + error.message);
      }
    };
  </script>
</body>
</html>
```

<details>
<summary>💡 Решение</summary>

```javascript
// api-wrapper.js

/**
 * Универсальная функция для API запросов
 */
async function apiRequest(url, options = {}) {
  const {
    method = 'GET',
    body = null,
    headers = {},
    timeout = 10000
  } = options;

  // Настройки fetch
  const fetchOptions = {
    method,
    headers: {
      'Content-Type': 'application/json',
      ...headers
    }
  };

  // Добавляем body для POST/PUT/PATCH
  if (body && ['POST', 'PUT', 'PATCH'].includes(method)) {
    fetchOptions.body = JSON.stringify(body);
  }

  // Timeout через AbortController
  const controller = new AbortController();
  fetchOptions.signal = controller.signal;

  const timeoutId = setTimeout(() => controller.abort(), timeout);

  try {
    const response = await fetch(url, fetchOptions);
    clearTimeout(timeoutId);

    // Обработка HTTP ошибок
    if (!response.ok) {
      let errorMessage = `HTTP ${response.status}: ${response.statusText}`;

      // Пытаемся получить детали ошибки от сервера
      try {
        const errorData = await response.json();
        errorMessage = errorData.message || errorMessage;
      } catch (e) {
        // Сервер не вернул JSON
      }

      throw new Error(errorMessage);
    }

    // Парсим ответ
    const contentType = response.headers.get('content-type');
    if (contentType && contentType.includes('application/json')) {
      return await response.json();
    } else {
      return await response.text();
    }

  } catch (error) {
    clearTimeout(timeoutId);

    // Обработка разных типов ошибок
    if (error.name === 'AbortError') {
      throw new Error('Превышено время ожидания ответа');
    } else if (error.name === 'TypeError') {
      throw new Error('Ошибка сети. Проверьте подключение к интернету');
    } else {
      throw error;
    }
  }
}

// Экспорт
export default apiRequest;

// Удобные хелперы
export const api = {
  get: (url, options) => apiRequest(url, { ...options, method: 'GET' }),
  post: (url, body, options) => apiRequest(url, { ...options, method: 'POST', body }),
  put: (url, body, options) => apiRequest(url, { ...options, method: 'PUT', body }),
  delete: (url, options) => apiRequest(url, { ...options, method: 'DELETE' })
};
```

**Использование с JSONPlaceholder API:**

```javascript
import apiRequest, { api } from './api-wrapper.js';

// GET запрос - получить всех пользователей
try {
  const users = await api.get('https://jsonplaceholder.typicode.com/users');
  console.log('Пользователей:', users.length);
  console.log('Первый пользователь:', users[0]);
} catch (error) {
  console.error('Ошибка:', error.message);
}

// GET запрос - получить конкретного пользователя
try {
  const user = await api.get('https://jsonplaceholder.typicode.com/users/1');
  console.log('Пользователь:', user.name);
} catch (error) {
  console.error('Ошибка:', error.message);
}

// POST запрос - создать пользователя
try {
  const newUser = await api.post('https://jsonplaceholder.typicode.com/users', {
    name: 'John Doe',
    email: 'john@example.com',
    username: 'johndoe'
  });
  console.log('Создан пользователь с ID:', newUser.id);
} catch (error) {
  console.error('Ошибка создания:', error.message);
}

// PUT запрос - обновить пользователя
try {
  const updated = await api.put('https://jsonplaceholder.typicode.com/users/1', {
    name: 'Jane Smith',
    email: 'jane@example.com'
  });
  console.log('Обновлён:', updated);
} catch (error) {
  console.error('Ошибка обновления:', error.message);
}

// DELETE запрос - удалить пользователя
try {
  await api.delete('https://jsonplaceholder.typicode.com/users/1');
  console.log('Пользователь удалён');
} catch (error) {
  console.error('Ошибка удаления:', error.message);
}

// С timeout
try {
  const posts = await apiRequest('https://jsonplaceholder.typicode.com/posts', {
    timeout: 5000
  });
  console.log('Постов загружено:', posts.length);
} catch (error) {
  console.error('Ошибка:', error.message);
}

// Обработка 404
try {
  await api.get('https://jsonplaceholder.typicode.com/users/999999');
} catch (error) {
  console.error('Ресурс не найден:', error.message); // HTTP 404
}
```

**Примечание:** JSONPlaceholder — это бесплатный fake REST API для тестирования. Он возвращает реалистичные ответы, но данные не сохраняются на сервере (POST/PUT/DELETE только эмулируются).

</details>

---

### Задание 2.2.6.2: Погодное API с кэшированием

Создай модуль для работы с OpenWeatherMap API с кэшированием.

**Задача:**
- Кэшировать запросы на 10 минут
- Показывать индикатор загрузки
- Обрабатывать ошибки сети и API

**Стартовый HTML:**

```html
<!DOCTYPE html>
<html lang="ru">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Погода с кэшированием</title>
  <style>
    body {
      font-family: Arial, sans-serif;
      max-width: 600px;
      margin: 50px auto;
      padding: 20px;
    }
    .weather-card {
      background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
      color: white;
      padding: 30px;
      border-radius: 16px;
      box-shadow: 0 10px 30px rgba(0,0,0,0.2);
    }
    .search {
      display: flex;
      gap: 10px;
      margin-bottom: 20px;
    }
    input {
      flex: 1;
      padding: 10px;
      border: none;
      border-radius: 4px;
    }
    button {
      padding: 10px 20px;
      border: none;
      border-radius: 4px;
      background: white;
      color: #667eea;
      font-weight: bold;
      cursor: pointer;
    }
    button:disabled {
      opacity: 0.5;
      cursor: not-allowed;
    }
    .weather-info {
      font-size: 18px;
    }
    .temp {
      font-size: 48px;
      font-weight: bold;
    }
    .cache-info {
      font-size: 12px;
      opacity: 0.8;
      margin-top: 10px;
    }
    .error {
      background: #f44336;
      padding: 15px;
      border-radius: 8px;
      margin-top: 20px;
    }
  </style>
</head>
<body>
  <div class="weather-card">
    <h1>🌤️ Погода с кэшем</h1>

    <div class="search">
      <input type="text" id="cityInput" placeholder="Введите город...">
      <button id="searchBtn">Поиск</button>
    </div>

    <div id="weatherInfo" class="weather-info"></div>
    <div id="cacheInfo" class="cache-info"></div>
  </div>

  <script type="module">
    // TODO: Импортируй apiRequest
    // import apiRequest from './api-wrapper.js';

    const API_KEY = 'твой_api_ключ'; // https://openweathermap.org/api
    const API_URL = 'https://api.openweathermap.org/data/2.5/weather';

    // TODO: Реализуй кэш
    // - Хранить данные в Map или объекте
    // - Ключ: название города
    // - Значение: { data, timestamp }
    // - Срок жизни: 10 минут

    // TODO: Реализуй функцию getWeather(city)
    // - Проверять кэш перед запросом
    // - Показывать, данные из кэша или свежие
    // - Обрабатывать ошибки

    // TODO: Реализуй UI функции
    // - showLoading()
    // - showWeather(data, fromCache)
    // - showError(message)

    document.getElementById('searchBtn').addEventListener('click', async () => {
      const city = document.getElementById('cityInput').value.trim();
      if (!city) return;

      // Твоя реализация
    });
  </script>
</body>
</html>
```

<details>
<summary>💡 Решение</summary>

```html
<!DOCTYPE html>
<html lang="ru">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Погода с кэшированием</title>
  <style>
    body {
      font-family: Arial, sans-serif;
      max-width: 600px;
      margin: 50px auto;
      padding: 20px;
      background: #f5f5f5;
    }
    .weather-card {
      background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
      color: white;
      padding: 30px;
      border-radius: 16px;
      box-shadow: 0 10px 30px rgba(0,0,0,0.2);
    }
    .search {
      display: flex;
      gap: 10px;
      margin-bottom: 20px;
    }
    input {
      flex: 1;
      padding: 10px;
      border: none;
      border-radius: 4px;
    }
    button {
      padding: 10px 20px;
      border: none;
      border-radius: 4px;
      background: white;
      color: #667eea;
      font-weight: bold;
      cursor: pointer;
      transition: all 0.3s;
    }
    button:hover:not(:disabled) {
      transform: scale(1.05);
    }
    button:disabled {
      opacity: 0.5;
      cursor: not-allowed;
    }
    .weather-info {
      font-size: 18px;
    }
    .temp {
      font-size: 48px;
      font-weight: bold;
      margin: 10px 0;
    }
    .cache-info {
      font-size: 12px;
      opacity: 0.8;
      margin-top: 15px;
      padding: 8px;
      background: rgba(255,255,255,0.1);
      border-radius: 4px;
    }
    .error {
      background: #f44336;
      padding: 15px;
      border-radius: 8px;
      margin-top: 20px;
    }
  </style>
</head>
<body>
  <div class="weather-card">
    <h1>🌤️ Погода с кэшем</h1>

    <div class="search">
      <input type="text" id="cityInput" placeholder="Введите город (Moscow, London...)">
      <button id="searchBtn">Поиск</button>
    </div>

    <div id="weatherInfo" class="weather-info"></div>
    <div id="cacheInfo" class="cache-info"></div>
  </div>

  <script>
    const API_KEY = 'твой_api_ключ'; // https://openweathermap.org/api
    const API_URL = 'https://api.openweathermap.org/data/2.5/weather';
    const CACHE_DURATION = 10 * 60 * 1000; // 10 минут

    // Кэш для запросов
    const cache = new Map();

    // Универсальная функция для API запросов
    async function apiRequest(url, options = {}) {
      const { timeout = 10000 } = options;

      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), timeout);

      try {
        const response = await fetch(url, {
          ...options,
          signal: controller.signal
        });

        clearTimeout(timeoutId);

        if (!response.ok) {
          if (response.status === 404) {
            throw new Error('Город не найден');
          } else if (response.status === 401) {
            throw new Error('Неверный API ключ');
          } else {
            throw new Error(`HTTP ${response.status}`);
          }
        }

        return await response.json();
      } catch (error) {
        clearTimeout(timeoutId);

        if (error.name === 'AbortError') {
          throw new Error('Превышено время ожидания');
        } else if (error.name === 'TypeError') {
          throw new Error('Ошибка сети. Проверьте подключение');
        }
        throw error;
      }
    }

    // Получение погоды с кэшированием
    async function getWeather(city) {
      const cacheKey = city.toLowerCase();

      // Проверяем кэш
      if (cache.has(cacheKey)) {
        const cached = cache.get(cacheKey);
        const age = Date.now() - cached.timestamp;

        // Если кэш свежий (< 10 минут)
        if (age < CACHE_DURATION) {
          console.log(`Данные из кэша (${Math.round(age / 1000)}с назад)`);
          return {
            data: cached.data,
            fromCache: true,
            cacheAge: age
          };
        }

        // Кэш устарел - удаляем
        cache.delete(cacheKey);
      }

      // Делаем запрос к API
      const url = `${API_URL}?q=${city}&appid=${API_KEY}&units=metric&lang=ru`;
      const data = await apiRequest(url, { timeout: 5000 });

      // Сохраняем в кэш
      cache.set(cacheKey, {
        data,
        timestamp: Date.now()
      });

      console.log('Свежие данные от API');
      return {
        data,
        fromCache: false
      };
    }

    // UI функции
    function showLoading() {
      const btn = document.getElementById('searchBtn');
      btn.disabled = true;
      btn.textContent = 'Загрузка...';

      document.getElementById('weatherInfo').innerHTML = '<p>⏳ Загрузка данных...</p>';
      document.getElementById('cacheInfo').textContent = '';
    }

    function showWeather(data, fromCache, cacheAge) {
      const { name, sys, main, weather, wind } = data;

      document.getElementById('weatherInfo').innerHTML = `
        <h2>${name}, ${sys.country}</h2>
        <div class="temp">${Math.round(main.temp)}°C</div>
        <p>${weather[0].description}</p>
        <p>Ощущается как: ${Math.round(main.feels_like)}°C</p>
        <p>Влажность: ${main.humidity}%</p>
        <p>Ветер: ${wind.speed} м/с</p>
      `;

      // Информация о кэше
      if (fromCache) {
        const seconds = Math.round(cacheAge / 1000);
        const minutes = Math.floor(seconds / 60);
        const timeStr = minutes > 0
          ? `${minutes} мин ${seconds % 60} сек`
          : `${seconds} сек`;

        document.getElementById('cacheInfo').innerHTML = `
          💾 Данные из кэша (${timeStr} назад)
        `;
      } else {
        document.getElementById('cacheInfo').innerHTML = `
          🌐 Свежие данные от API<br>
          Будут кэшированы на 10 минут
        `;
      }
    }

    function showError(message) {
      document.getElementById('weatherInfo').innerHTML = `
        <div class="error">
          ❌ ${message}
        </div>
      `;
      document.getElementById('cacheInfo').textContent = '';
    }

    function resetButton() {
      const btn = document.getElementById('searchBtn');
      btn.disabled = false;
      btn.textContent = 'Поиск';
    }

    // Обработчик поиска
    document.getElementById('searchBtn').addEventListener('click', async () => {
      const city = document.getElementById('cityInput').value.trim();

      if (!city) {
        showError('Введите название города');
        return;
      }

      showLoading();

      try {
        const { data, fromCache, cacheAge } = await getWeather(city);
        showWeather(data, fromCache, cacheAge);
      } catch (error) {
        showError(error.message);
      } finally {
        resetButton();
      }
    });

    // Enter в input
    document.getElementById('cityInput').addEventListener('keypress', (e) => {
      if (e.key === 'Enter') {
        document.getElementById('searchBtn').click();
      }
    });

    // Загрузка Москвы по умолчанию
    document.getElementById('cityInput').value = 'Moscow';
    document.getElementById('searchBtn').click();
  </script>
</body>
</html>
```

**Как работает кэш:**
1. При первом запросе данные загружаются от API и сохраняются в Map
2. При повторном запросе проверяется возраст кэша
3. Если кэшу < 10 минут — возвращаются данные из кэша
4. Если кэш устарел — делается новый запрос

**Проверь:** Найди погоду для города, затем найди его снова — второй запрос вернётся мгновенно из кэша!

</details>

---

### Задание 2.2.6.3: Множественные запросы

Загрузи погоду для нескольких городов параллельно.

**Задача:** Используй `Promise.all` для параллельной загрузки погоды.

**Стартовый код:**

```javascript
const API_KEY = 'твой_api_ключ';
const API_URL = 'https://api.openweathermap.org/data/2.5/weather';

// TODO: Реализуй функцию загрузки погоды для одного города
async function fetchCityWeather(city) {
  // Твоя реализация
}

// TODO: Реализуй функцию загрузки для нескольких городов
// Используй Promise.all для параллельной загрузки
async function fetchMultipleCities(cities) {
  // Твоя реализация
}

// Использование
const cities = ['Moscow', 'London', 'Paris', 'Tokyo', 'New York'];
// const results = await fetchMultipleCities(cities);
```

<details>
<summary>💡 Решение</summary>

```javascript
const API_KEY = 'твой_api_ключ';
const API_URL = 'https://api.openweathermap.org/data/2.5/weather';

// Загрузка погоды для одного города
async function fetchCityWeather(city) {
  try {
    const url = `${API_URL}?q=${city}&appid=${API_KEY}&units=metric&lang=ru`;
    const response = await fetch(url);

    if (!response.ok) {
      throw new Error(`${city}: город не найден`);
    }

    const data = await response.json();

    return {
      success: true,
      city: data.name,
      temp: Math.round(data.main.temp),
      description: data.weather[0].description,
      icon: data.weather[0].icon
    };
  } catch (error) {
    return {
      success: false,
      city,
      error: error.message
    };
  }
}

// Загрузка для нескольких городов параллельно
async function fetchMultipleCities(cities) {
  console.log(`Загрузка погоды для ${cities.length} городов...`);

  const startTime = Date.now();

  // Promise.all - все запросы параллельно
  const results = await Promise.all(
    cities.map(city => fetchCityWeather(city))
  );

  const duration = Date.now() - startTime;
  console.log(`Загрузка завершена за ${duration}ms`);

  // Разделяем успешные и неудачные
  const successful = results.filter(r => r.success);
  const failed = results.filter(r => !r.success);

  return {
    successful,
    failed,
    duration
  };
}

// Использование
async function demo() {
  const cities = ['Moscow', 'London', 'Paris', 'Tokyo', 'New York'];

  const { successful, failed, duration } = await fetchMultipleCities(cities);

  console.log('\n✅ Успешно загружено:', successful.length);
  successful.forEach(result => {
    console.log(`  ${result.city}: ${result.temp}°C, ${result.description}`);
  });

  if (failed.length > 0) {
    console.log('\n❌ Ошибки:', failed.length);
    failed.forEach(result => {
      console.log(`  ${result.error}`);
    });
  }

  console.log(`\n⏱ Общее время: ${duration}ms`);
}

demo();
```

**Вариант с Promise.allSettled (безопаснее):**

```javascript
// Promise.allSettled - не останавливается при ошибке
async function fetchMultipleCitiesSafe(cities) {
  const results = await Promise.allSettled(
    cities.map(city => fetchCityWeather(city))
  );

  return results.map((result, i) => {
    if (result.status === 'fulfilled') {
      return result.value;
    } else {
      return {
        success: false,
        city: cities[i],
        error: result.reason.message
      };
    }
  });
}
```

**Разница:**
- `Promise.all` — если один запрос упал, всё падает
- `Promise.allSettled` — всегда ждёт все запросы, даже если есть ошибки

</details>

---

## 🏆 Итоговое задание 2.2.6

**Создай полноценное погодное приложение** с поиском, избранным и сравнением городов.

### Требования:

1. **Функциональность:**
   - Поиск погоды по названию города
   - Получение погоды по геолокации
   - Добавление городов в избранное (localStorage)
   - Сравнение погоды в нескольких городах
   - Кэширование запросов

2. **Обработка ошибок:**
   - Город не найден
   - Проблемы с сетью
   - Неверный API ключ
   - Timeout запросов

3. **UI/UX:**
   - Индикаторы загрузки
   - Информация о кэше
   - Адаптивная вёрстка
   - Анимации

**Структура проекта:**

```
weather-app/
├── index.html
├── modules/
│   ├── api.js          # API wrapper + кэширование
│   ├── weather.js      # Работа с погодным API
│   ├── storage.js      # LocalStorage (избранное)
│   └── ui.js           # UI компоненты
├── app.js              # Главный файл
└── style.css           # Стили
```

<details>
<summary>💡 Подсказки по архитектуре</summary>

### modules/api.js
```javascript
// Универсальная обёртка для fetch с:
// - Timeout
// - Обработкой ошибок
// - Retry логикой (опционально)

export async function apiRequest(url, options = {}) {
  // Реализация из задания 2.2.6.1
}

export const cache = new Map();
export const CACHE_DURATION = 10 * 60 * 1000;

export function getCached(key) {
  if (!cache.has(key)) return null;

  const { data, timestamp } = cache.get(key);
  const age = Date.now() - timestamp;

  if (age > CACHE_DURATION) {
    cache.delete(key);
    return null;
  }

  return { data, fromCache: true, age };
}

export function setCache(key, data) {
  cache.set(key, { data, timestamp: Date.now() });
}
```

### modules/weather.js
```javascript
import { apiRequest, getCached, setCache } from './api.js';

const API_KEY = 'твой_ключ';
const API_URL = 'https://api.openweathermap.org/data/2.5/weather';

export async function getWeatherByCity(city) {
  const cacheKey = `weather:${city.toLowerCase()}`;

  // Проверка кэша
  const cached = getCached(cacheKey);
  if (cached) return cached;

  // Запрос к API
  const url = `${API_URL}?q=${city}&appid=${API_KEY}&units=metric&lang=ru`;
  const data = await apiRequest(url);

  // Сохранение в кэш
  setCache(cacheKey, data);

  return { data, fromCache: false };
}

export async function getWeatherByCoords(lat, lon) {
  // Реализация для геолокации
}

export async function getWeatherForMultipleCities(cities) {
  return Promise.all(cities.map(city => getWeatherByCity(city)));
}
```

### modules/storage.js
```javascript
const FAVORITES_KEY = 'weather_favorites';

export function getFavorites() {
  const data = localStorage.getItem(FAVORITES_KEY);
  return data ? JSON.parse(data) : [];
}

export function addFavorite(city) {
  const favorites = getFavorites();
  if (!favorites.includes(city)) {
    favorites.push(city);
    localStorage.setItem(FAVORITES_KEY, JSON.stringify(favorites));
  }
}

export function removeFavorite(city) {
  let favorites = getFavorites();
  favorites = favorites.filter(c => c !== city);
  localStorage.setItem(FAVORITES_KEY, JSON.stringify(favorites));
}

export function isFavorite(city) {
  return getFavorites().includes(city);
}
```

### modules/ui.js
```javascript
export function renderWeatherCard(data, container) {
  // Создание карточки с погодой
}

export function showLoading(container) {
  container.innerHTML = '<div class="loading">⏳ Загрузка...</div>';
}

export function showError(container, message) {
  container.innerHTML = `<div class="error">❌ ${message}</div>`;
}

export function renderFavorites(cities, onRemove) {
  // Отрисовка списка избранного
}

export function renderComparison(cities, onClose) {
  // Отрисовка сравнения городов
}
```

### app.js
```javascript
import { getWeatherByCity, getWeatherForMultipleCities } from './modules/weather.js';
import { getFavorites, addFavorite, removeFavorite } from './modules/storage.js';
import { renderWeatherCard, renderFavorites } from './modules/ui.js';

// Инициализация приложения
async function init() {
  setupEventListeners();
  await loadFavorites();
  await loadDefaultCity();
}

function setupEventListeners() {
  // Поиск
  document.getElementById('searchBtn').addEventListener('click', handleSearch);

  // Геолокация
  document.getElementById('geoBtn').addEventListener('click', handleGeoLocation);

  // Избранное
  document.getElementById('favBtn').addEventListener('click', handleFavoriteToggle);
}

init();
```

</details>

---

## ✅ Чеклист

После изучения этой темы ты должен уметь:

- [ ] Понимать, что такое API и REST
- [ ] Делать GET, POST, PUT, DELETE запросы через fetch
- [ ] Работать с заголовками и телом запроса
- [ ] Правильно обрабатывать ошибки (сеть + HTTP)
- [ ] Парсить разные типы ответов (JSON, текст, blob)
- [ ] Использовать AbortController для timeout и отмены
- [ ] Понимать CORS и способы решения проблем
- [ ] Кэшировать запросы для оптимизации
- [ ] Делать параллельные запросы через Promise.all

---

## 🔗 Ресурсы

**Статьи:**
- [MDN: Fetch API](https://developer.mozilla.org/ru/docs/Web/API/Fetch_API)
- [MDN: Using Fetch](https://developer.mozilla.org/ru/docs/Web/API/Fetch_API/Using_Fetch)
- [JavaScript.info: Fetch](https://learn.javascript.ru/fetch)

**API для практики:**
- [OpenWeatherMap](https://openweathermap.org/api) — погода
- [JSONPlaceholder](https://jsonplaceholder.typicode.com/) — фейковый REST API для тестов
- [Rick and Morty API](https://rickandmortyapi.com/) — бесплатный REST API
- [PokéAPI](https://pokeapi.co/) — данные о покемонах

**Инструменты:**
- [Postman](https://www.postman.com/) — тестирование API
- [HTTPie](https://httpie.io/) — CLI для HTTP запросов
- [Insomnia](https://insomnia.rest/) — альтернатива Postman

**Видео:**
- [Fetch API — Владилен Минин](https://www.youtube.com/watch?v=eKCD9djJQKc)
- [Working with APIs — Web Dev Simplified](https://www.youtube.com/watch?v=cuEtnrL9-H0)

---

⏱ **Время на изучение**: 4-5 часов

---

**Предыдущая тема:** [2.2.5 Модули (ES6+)](./2.2.5-Модули.md)

**Следующая тема:** [2.2.7 Обработка ошибок](./2.2.7-Обработка-ошибок.md)

...
Запусти приложение и найди любой город:

```bash
npm run dev
```

Открой консоль браузера (F12 → Console) и следи за переходами состояний: при нажатии кнопки поиска должно пройти `idle` → `loading` → `success` (или `error` если город не найден), а блок с погодой должен обновляться соответственно.
...
TypeScript не даст вызвать `setState` с неправильным значением:
```typescript
setState("pending"); // ❌ Argument of type '"pending"' is not assignable
```

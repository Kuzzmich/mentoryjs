# База знаний для Fullstack-разработчиков

Привет! 👋

Это база знаний для начинающих и растущих разработчиков. Здесь собраны материалы по стеку **Node.js + Vue.js + MongoDB** — от основ до продвинутых тем.

---

На каждой странице тебя ждёт:
- короткое объяснение без воды,
- практические задания для закрепления,
- ссылки на документацию, статьи и видео.

---

Изучай в своём темпе. Не обязательно идти по порядку — выбирай то, что актуально сейчас.

Если материал ещё в работе — он будет со значком 🔒.

---

> 💬 **Нет материала, который хочешь изучить?**  
> Напиши — добавлю в ближайшее время.

---

## 🟢 Основы программирования
*Для тех, кто начинает с нуля*

→ [Перейти к разделу Основы](./01-Основы/README.md)

- [Как работает компьютер и интернет](./01-Основы/1.1-Компьютер-и-интернет.md)
- [Терминал / командная строка](./01-Основы/1.2.1-Терминал.md)
- [Git и GitHub](./01-Основы/1.2.2-Git-и-GitHub.md)
- [VS Code: настройка и расширения](./01-Основы/1.2.3-VS-Code.md)
- [JetBrains WebStorm](./01-Основы/1.2.4-WebStorm.md)
- [DevTools браузера](./01-Основы/1.2.5-DevTools.md)

---

## 🟡 JavaScript
*Ядро всего стека*

→ [Перейти к разделу JavaScript](./02-JavaScript/README.md)

**Основы**
- [Переменные, типы данных, операторы](./02-JavaScript/2.1.1-Переменные-типы-операторы.md)
- [Массивы и объекты](./02-JavaScript/2.1.2-Массивы-и-объекты.md)
- [Условия и циклы](./02-JavaScript/2.1.3-Условия-и-циклы.md)
- [Функции и области видимости](./02-JavaScript/2.1.4-Функции-и-области-видимости.md)
- [DOM и события](./02-JavaScript/2.1.5-DOM-и-события.md)

**Продвинутый уровень**
- [Замыкания и контекст (this)](./02-JavaScript/2.2.1-Замыкания-и-контекст.md)
- [Прототипы и классы](./02-JavaScript/2.2.2-Прототипы-и-классы.md)
- [Асинхронность: callbacks → promises → async/await](./02-JavaScript/2.2.3-Асинхронность.md)
- [Работа с API (fetch)](./02-JavaScript/2.2.4-Работа-с-API.md)
- [Event Loop](./02-JavaScript/2.2.5-Event-Loop.md)
- [Модули (ES6+)](./02-JavaScript/2.2.6-Модули.md)
- [Обработка ошибок](./02-JavaScript/2.2.7-Обработка-ошибок.md)

**Экспертный уровень**
- 🔒 Паттерны проектирования
- 🔒 Функциональное программирование
- 🔒 Генераторы и итераторы
- 🔒 Proxy, Reflect, WeakMap

---

## 🔵 TypeScript

→ [Перейти к разделу TypeScript](./03-TypeScript/README.md)

**Основы**
- [Зачем TypeScript](./03-TypeScript/3.1.1-Зачем-TypeScript.md)
- [Настройка TypeScript в проектах](./03-TypeScript/3.1.2-Настройка-TypeScript.md)
- [Бандлеры и Vite](./03-TypeScript/3.1.3-Бандлеры-и-Vite.md)
- [Базовые типы](./03-TypeScript/3.1.4-Базовые-типы.md)
- [Интерфейсы vs Type aliases](./03-TypeScript/3.1.5-Интерфейсы-vs-Type-aliases.md)
- [Функции с типами](./03-TypeScript/3.1.6-Функции-с-типами.md)

**Продвинутый уровень**
- [Классы и модификаторы](./03-TypeScript/3.2.1-Классы-и-модификаторы.md)
- [Generics](./03-TypeScript/3.2.2-Generics.md)
- [Utility Types](./03-TypeScript/3.2.3-Utility-Types.md)
- [Type Guards и Narrowing](./03-TypeScript/3.2.4-Type-Guards-и-Narrowing.md)
- [Conditional Types](./03-TypeScript/3.2.5-Conditional-Types.md)
- [Mapped Types](./03-TypeScript/3.2.6-Mapped-Types.md)
- [Template Literal Types](./03-TypeScript/3.2.7-Template-Literal-Types.md)
- [Декораторы и Metadata](./03-TypeScript/3.2.8-Декораторы-и-Metadata.md)
- [Declaration Merging и Module Augmentation](./03-TypeScript/3.2.9-Declaration-Merging.md)
- [Продвинутая настройка и миграция](./03-TypeScript/3.2.10-Продвинутая-настройка.md)

---

## 🟢 Vue.js

→ [Перейти к разделу Vue.js](./04-Vue/README.md)

**Основы**
- [Введение в Vue 3 — SFC, Vite, первый компонент](./04-Vue/4.1.1-Введение-в-Vue.md)
- [Реактивность: ref, reactive, computed](./04-Vue/4.1.2-Реактивность.md)
- [Template и директивы: v-if, v-for, v-model, v-bind, v-on](./04-Vue/4.1.3-Template-и-директивы.md)
- [Компоненты: props и emit](./04-Vue/4.1.4-Компоненты-props-emit.md)
- [Lifecycle hooks](./04-Vue/4.1.5-Lifecycle-hooks.md)
- [Watchers: watch и watchEffect](./04-Vue/4.1.6-Watchers.md)

**Продвинутый уровень**
- [Composables](./04-Vue/4.2.1-Composables.md)
- [Слоты и динамические компоненты](./04-Vue/4.2.2-Слоты.md)
- [Vue Router](./04-Vue/4.2.3-Vue-Router.md)
- [Pinia (state management)](./04-Vue/4.2.4-Pinia.md)
- [Vue + TypeScript углублённо](./04-Vue/4.2.5-Vue-TypeScript.md)

**Экспертный уровень**
- [Оптимизация рендеринга](./04-Vue/4.3.1-Оптимизация-рендеринга.md)
- [Тестирование (Vitest + Vue Test Utils)](./04-Vue/4.3.2-Тестирование.md)
- [SSR с Nuxt](./04-Vue/4.3.3-SSR-Nuxt.md)

---

## 🟣 Node.js

→ [Перейти к разделу Node.js](./05-Node/README.md)

*Весь раздел написан на TypeScript. Сквозной проект — Weather API: бэкенд для погодного приложения.*

**Основы**
- [Что такое Node.js](./05-Node/5.1.1-Что-такое-Node.md)
- [Модули и окружение](./05-Node/5.1.2-Модули-и-окружение.md)

**Express + TypeScript**
- [Express + TypeScript: инициализация проекта](./05-Node/5.2.1-Express-TypeScript.md)
- [Роутинг и структура проекта](./05-Node/5.2.2-Роутинг-и-структура.md)
- [Middleware](./05-Node/5.2.3-Middleware.md)
- [Валидация (Zod)](./05-Node/5.2.4-Валидация-Zod.md)
- [Аутентификация (JWT)](./05-Node/5.2.5-Аутентификация-JWT.md)
- [Загрузка файлов (Multer)](./05-Node/5.2.6-Загрузка-файлов.md)

**Тестирование**
- [Тестирование API (Vitest + Supertest)](./05-Node/5.3.1-Тестирование-API.md)

**Экспертный уровень**
- [WebSockets](./05-Node/5.4.1-WebSockets.md)
- 🔒 Streams и Buffer
- 🔒 Docker + деплой

---

## 🟠 MongoDB

**Основы**
- 🔒 Документо-ориентированные БД
- 🔒 CRUD операции
- 🔒 Mongoose ODM
- 🔒 Схемы и модели
- 🔒 Индексы

**Продвинутый уровень**
- 🔒 Aggregation Pipeline
- 🔒 Связи между документами
- 🔒 Транзакции
- 🔒 Валидация на уровне схемы

**Экспертный уровень**
- 🔒 Шардинг и репликация
- 🔒 Оптимизация запросов
- 🔒 Паттерны моделирования

---

## 🔧 DevOps & Инфраструктура

- 🔒 Docker и docker-compose
- 🔒 CI/CD (GitHub Actions)
- 🔒 Nginx
- 🔒 Деплой (VPS, Vercel, Railway)

---

## 🎯 Софт-скиллы

- 🔒 Code Review: как давать и принимать
- 🔒 Работа с задачами
- 🔒 Оценка сроков
- 🔒 Технический долг
- 🔒 Документирование кода

---

## 💼 Карьера

- 🔒 Как составить резюме
- 🔒 GitHub-профиль
- 🔒 Подготовка к собеседованиям
- 🔒 Тестовые задания
- 🔒 Как торговаться по зарплате

---

[//]: # (## 📋 Чеклисты)

[//]: # ()
[//]: # (- [Junior → Middle]&#40;./checklist-junior-middle.md&#41;)

[//]: # (- [Middle → Senior]&#40;./checklist-middle-senior.md&#41;)

[//]: # ()
[//]: # (---)

## 🛠 Практические проекты

| Уровень | Проект |
|---------|--------|
| Junior | Погодное приложение с API |
| Junior | 🔒 Блог с авторизацией |
| Middle | 🔒 Чат на WebSockets |
| Middle | 🔒 Дашборд с аналитикой |
| Senior | 🔒 SaaS с мультитенантностью |
